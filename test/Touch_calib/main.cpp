...
uint16_t touchScreenMinimumX = 200, touchScreenMaximumX = 3700,
         touchScreenMinimumY = 240,
         touchScreenMaximumY =
             3800; // Chạy Calibration để lấy giá trị mỗi màn hình mỗi khác
...
void my_touch_read(lv_indev_t *indev_drv, lv_indev_data_t *data) {
  if (ts.touched()) {
    ...
    data->point.x = map(p.x, touchScreenMinimumX, touchScreenMaximumX, 1,
                        screenWidth); /* Touchscreen X calibration */
    data->point.y = map(p.y, touchScreenMinimumY, touchScreenMaximumY, 1,
                        screenHeight); /* Touchscreen Y calibration */
...
void LED_RGB() {
  /*Lấy 255 trừ đi giá trị slider bởi vì LED này đấu cực dương chung  (LED
   * common anode)*/
...
void setup() {
  ...
  tsSpi.begin(XPT2046_CLK, XPT2046_MISO, XPT2046_MOSI,
              XPT2046_CS); /* Start second SPI bus for touchscreen */
