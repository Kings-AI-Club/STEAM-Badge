...
def draw_image(x, y, img_data):
    ...
    img_data: (width, height, pixel_bytes) as returned by load_image()
    """
    w, h, data = img_data
    if display._full_fb:
        # Blit pixel by pixel into framebuffer
        idx = 0
        for py in range(h):
            for px in range(w):
                if idx + 1 < len(data):
                    # framebuf RGB565 is native byte order
                    c = data[idx] | (data[idx + 1] << 8)
                    if c != 0:  # skip transparent (black) pixels
...
class TouchButton:
    ...
    def draw(self, active=False):
        ...
        display.rounded_rect(self.x, self.y, self.w, self.h, 4, c)
        if self.label:
    ...
    def check_touch(self, touch_pos):
        """Check if a touch position hits this button. Returns bool."""
...
def run_app(update_fn, init_fn=None, target_fps=30):
    """
    Main app run loop.
    - update_fn(touch_pos): called every frame with current touch pos (or None)
